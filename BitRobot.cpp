 ...
 * 读取函数，否则就会把10ms全部占用了
 */
#define SCHED_TASK(func) (void (*)())&Robot::func

const BIT_Scheduler::Task Robot::scheduler_tasks[] =
{
    //真正读取传感器函数
    { SCHED_TASK(read_device_gps_JY901),                                        1,      200 },
    { SCHED_TASK(read_device_gps_UM220),                                        1,      200 },

    //真正写入外部设备的函数，比如设置继电器让方向舵切换左右转
    { SCHED_TASK(write_device_servos_output),                                  20,     1000 },
    ...
    { SCHED_TASK(update_all_external_device_output),                           10,      100 },

    { SCHED_TASK(get_gcs_udp),                                                 10,      200 },
    { SCHED_TASK(update_navigation_loop),                                      10,      250 },
    { SCHED_TASK(send_ap2gcs_realtime_data_RobotLink_by_udp),                   1,      350 },
...
int main(int argc,char * const argv[])
{
    ...
    robot.scheduler.init(&robot.scheduler_tasks[0], sizeof(robot.scheduler_tasks)/sizeof(robot.scheduler_tasks[0])); // 初始化任务调度表
    DEBUG_PRINTF("There are %d tasks to run !!! \n",sizeof(robot.scheduler_tasks) / sizeof(robot.scheduler_tasks[0]));

    robot.setup(); // 初始化步骤，初始化一些设备或者参数等
